...
def cut_ts(timeseries, window_size, gap):
    """Make input ts and prediction targets from a `timeseries` for use in machine learning.
    ...
    X = np.atleast_3d(np.array([timeseries[start:start + window_size] for start in range(0, timeseries.shape[0] - window_size - 1, gap)]))
    y = [timeseries[start + window_size + 1] for start in range(0, timeseries.shape[0] - window_size - 1, gap)]
